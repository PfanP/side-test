...
// AppendReviewByCreatorMovieID appends a review in the store with a creator and title
func (k Keeper) AppendReviewByCreatorMovieID(
	ctx sdk.Context,
	creator string,
	movieId uint64,
) {
	...
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.ReviewCreatorIDKey))
	id := GetCreatorMovieID(creator, movieId)
	store.Set(id, []byte(creator))
}
...
// GetReviewByCreatorMovieID returns a review from its id
func (k Keeper) GetReviewByCreatorMovieID(ctx sdk.Context, creator string, id uint64) bool {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.ReviewCreatorIDKey))
	b := store.Get(GetCreatorMovieID(creator, id))
	if b == nil {
		return false
	}
	return true
...
// RemoveReview removes a review from the store
func (k Keeper) RemoveReviewByCreatorMovieID(ctx sdk.Context, creator string, id uint64) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.ReviewKey))
	store.Delete(GetCreatorMovieID(creator, id))
}
...
func GetCreatorMovieID(creator string, id uint64) []byte {
	creatorByte := []byte(fmt.Sprintf("%s%d",creator, id))
	return creatorByte
}

...
func TestReviewMsgServerCreate(t *testing.T) {
	...
	creator := "A"
	for i := 0; i < 5; i++ {
		resp, err := srv.CreateReview(ctx, &types.MsgCreateReview{Creator: creator, MovieId: uint64(i)})
		require.NoError(t, err)
		require.Equal(t, i, int(resp.Id))
	}
}

func TestReviewMsgServerDuplicated(t *testing.T) {
	srv, ctx := setupMsgServer(t)
	creator := "A"
	resp, err := srv.CreateReview(ctx, &types.MsgCreateReview{Creator: creator, MovieId: 10})
	require.NoError(t, err)

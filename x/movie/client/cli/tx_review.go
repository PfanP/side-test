...
func CmdCreateReview() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create-review [movie-id] [rating] [description]",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			argRating, err := cast.ToUint64E(args[1])
			if err != nil {
				return err
			}
			...
			msg := types.NewMsgCreateReview(clientCtx.GetFromAddress().String(), argMovieId, argRating, argDescription)
...
func CmdUpdateReview() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update-review [id] [movie-id] [rating] [description]",
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			msg := types.NewMsgUpdateReview(clientCtx.GetFromAddress().String(), id, argMovieId, argRating, argDescription)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
